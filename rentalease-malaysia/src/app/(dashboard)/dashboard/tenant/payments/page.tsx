...
export default async function TenantPaymentsPage() {
  ...
  const tenancy = await prisma.tenancy.findFirst({
    ...
    include: {
      ...
      rentPayments: {
        orderBy: { dueDate: 'asc' },
        include: {
          // Include proofs so we can show thumbnails and notification dots
          proofs: {
  ...
  // Count unread landlord decisions (isReadByTenant = false means
  // the landlord made a decision the tenant hasn't "seen" yet)
  ...
  // Mark all proofs as read now that the tenant is viewing this page
  // We do this as a fire-and-forget — no need to await
  if (unreadDecisions > 0 && tenancy) {
    ...
    prisma.paymentProof
      ...
      .catch(console.error); // non-blocking, non-critical
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">
              Upload proof of payment for each month and track verification
              status.
            </p>
          </div>
          {unreadDecisions > 0 && (
